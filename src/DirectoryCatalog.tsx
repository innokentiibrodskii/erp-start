      ...
      {form.open && (
        <BottomSheet onClose={close}>
          ...
          <div className="px-5 space-y-4">
            ...
            {entityType === 'material' && (
              <label className="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3 cursor-pointer">
                <span className="text-sm text-slate-600">{t('directory.showInMaterialUsage')}</span>
                <input type="checkbox" checked={form.showInMaterialUsage} onChange={e => setForm(f => ({ ...f, showInMaterialUsage: e.target.checked }))}
                  className="h-4 w-4 rounded accent-slate-800" />
              </label>
            )}
          </div>
