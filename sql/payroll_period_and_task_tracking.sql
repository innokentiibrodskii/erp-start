...
--
-- Кожна команда навмисно в один рядок (крім тіл функцій) — попередні спроби
-- показали, що копіювання багаторядкових команд у SQL Editor іноді губить
-- перший рядок/частину дужок. Один рядок = нічого губити.
--
-- Що робить:
...
alter table organizations add constraint organizations_payroll_period_check check ((payroll_open_from_day is null and payroll_open_to_day is null) or (payroll_open_from_day between 1 and 31 and payroll_open_to_day between 1 and 31 and payroll_open_from_day <= payroll_open_to_day));
...
alter table assignments add constraint assignments_priority_check check (priority in ('low', 'medium', 'high', 'urgent'));
...
create policy payroll_period_closures_select on payroll_period_closures for select using (organization_id in (select organization_id from user_organizations where user_id = auth.uid()));
drop policy if exists payroll_period_closures_insert on payroll_period_closures;
create policy payroll_period_closures_insert on payroll_period_closures for insert with check (closed_by = auth.uid() and organization_id in (select organization_id from user_organizations where user_id = auth.uid()) and exists (select 1 from users where id = auth.uid() and role = 'admin'));
...
create table if not exists assignment_events (
  ...
  event_type text not null check (event_type in ('created', 'status_changed', 'duration_changed', 'cost_changed', 'priority_changed', 'due_date_changed', 'product_changed')),
  old_value jsonb,
  new_value jsonb,
...
alter table assignment_events add constraint assignment_events_event_type_check check (event_type in ('created', 'status_changed', 'duration_changed', 'cost_changed', 'priority_changed', 'due_date_changed', 'product_changed'));
...
create policy assignment_events_select on assignment_events for select using (exists (select 1 from assignments a where a.id = assignment_events.assignment_id and (a.assignee_id = auth.uid() or exists (select 1 from users u where u.id = auth.uid() and u.role in ('manager', 'admin')))));
...
create trigger assignment_events_insert_trg after insert on assignments for each row execute function log_assignment_event();
drop trigger if exists assignment_events_update_trg on assignments;
create trigger assignment_events_update_trg after update on assignments for each row execute function log_assignment_event();
...
begin
  ...
  select payroll_open_to_day into v_open_to_day from organizations where id = old.organization_id;

  v_closed := exists (select 1 from payroll_period_closures where organization_id = old.organization_id and period_year = extract(year from v_completed_day)::int and period_month = extract(month from v_completed_day)::int);
  ...
  if v_open_to_day is not null then
    v_period_end_day := least((date_trunc('month', v_completed_day) + interval '1 month' - interval '1 day')::date, (date_trunc('month', v_completed_day) + ((v_open_to_day - 1) * interval '1 day'))::date);
  end if;

  -- Статус — без змін від наявної поведінки: лише в день завершення.
  if new.status is distinct from old.status then
    if v_closed or v_today <> v_completed_day then
      raise exception 'Завершене завдання можна редагувати лише в день завершення' using errcode = '22023';
    end if;
  end if;

  -- Час і вартість — день завершення, АБО останній день зарплатного періоду
  -- (якщо він налаштований), і період ще не закритий вручну.
  if new.duration_minutes is distinct from old.duration_minutes or new.cost is distinct from old.cost then
    if v_closed then
      raise exception 'Зарплатний період закрито — час і вартість завдання більше не редагуються' using errcode = '22023';
    end if;
    if v_today <> v_completed_day and (v_open_to_day is null or v_today <> v_period_end_day) then
      raise exception 'Час і вартість завершеного завдання можна редагувати лише в день завершення або в останній день зарплатного періоду' using errcode = '22023';
    end if;
  end if;
...
create trigger assignments_lock_after_completion_trg before update on assignments for each row execute function assignments_lock_after_completion();
...
--
--   select column_name from information_schema.columns where table_name in ('organizations','assignments') and column_name in ('payroll_open_from_day','payroll_open_to_day','priority','due_date');
--
--   select trigger_name, event_manipulation from information_schema.triggers where event_object_table = 'assignments';
-- ============================================================================
