-- ============================================================================
-- Завдання без виконавця (assignments.assignee_id стає необов'язковим)
-- ============================================================================
...
--
-- Що робить:
-- 1. Знімає обмеження NOT NULL з assignments.assignee_id — тепер можна
--    створити завдання без виконавця й призначити його пізніше (сторінка
--    "Завдання" → "Нове" / "Дані для менеджера").
-- 2. Дозволяє новий тип події 'assignee_changed' і додає його логування
--    в тригер log_assignment_event() — призначення/зміна виконавця тепер
--    теж потрапляє в "Історію" завдання.
-- ============================================================================


alter table assignments alter column assignee_id drop not null;


alter table assignment_events drop constraint if exists assignment_events_event_type_check;
alter table assignment_events add constraint assignment_events_event_type_check check (event_type in ('created', 'status_changed', 'duration_changed', 'cost_changed', 'priority_changed', 'due_date_changed', 'product_changed', 'planned_duration_changed', 'name_changed', 'assignee_changed'));
...
begin
  ...
  if tg_op = 'UPDATE' then
    ...
    if new.assignee_id is distinct from old.assignee_id then
      insert into assignment_events (assignment_id, organization_id, actor_id, event_type, old_value, new_value)
      values (new.id, new.organization_id, auth.uid(), 'assignee_changed', to_jsonb(old.assignee_id), to_jsonb(new.assignee_id));
    end if;
    return new;
  end if;
